...
| 2026-07-15 | squad:validation[:500] | recall@3 = 292/493 | 59% | First real (non-synthetic) dataset: SQuAD v1.1 from Hugging Face (`rajpurkar/squad`), loaded via `eval/datasets/squad.py` (needs `pip install corticore[hf]`). 500 rows shuffled with `seed=42` → 2337 context sentences stored as memories, 493 answerable questions recalled against them with the default `LocalEmbedder`. Command: `PYTHONPATH=src python eval/harness.py --dataset squad --limit 500`. This is a genuine retrieval-among-distractors number; expect semantic embedders (`[st]`) to raise it, as on the synthetic set. |
